...
namespace OrderApi.Application.Services
{
    public class OrderService(IOrder orderInterface, HttpClient httpClient, ResiliencePipelineProvider<string> resiliencePipeline) : IOrderService
    {
        ...
        {
            var getProductById = await httpClient.GetAsync($"/api/Products/{productId}");
        ...
        {
            var getAppUserById = await httpClient.GetAsync($"http://localhost:5000/api/Authentication/{appUserId}");
